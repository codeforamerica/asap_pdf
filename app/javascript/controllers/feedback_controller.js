  ...
  async patchFeedback() {
    ...
    try {
      ...
      this.getInferences(false).map((inference_id) => {
        deleteData["feedback_items"].push({"document_inference_id": inference_id})
      })
      ...
      this.getInferences(true).map((inference_id) => {
        patchData["feedback_items"].push(Object.assign({"document_inference_id": inference_id}, this.store))
      })
