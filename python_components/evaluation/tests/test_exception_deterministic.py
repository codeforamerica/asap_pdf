...
def test_creation_date_fuzzy_search():
    ...
    result = deterministic_score.evaluate_created_date(doc.created_date,
                                                       doc.ai_exception["why_archival"])
    assert type(result) is dict
    assert result["score"] == 0
    assert result["reason"] == "Created date was not found in explanation."

    doc.ai_exception["why_archival"] += " The document was created on December 12 2024, which is recent history."
    result = deterministic_score.evaluate_created_date(doc.created_date,
                                                       doc.ai_exception["why_archival"])
    assert result["score"] == 0
    assert result["reason"] == "Created date was not found in explanation."

    doc.ai_exception["why_archival"] += " The document was created on January 2025, which is recent history."
    result = deterministic_score.evaluate_created_date(doc.created_date,
                                                       doc.ai_exception["why_archival"])
...
def test_creation_date_spacy():
    ...
    result = deterministic_score.evaluate_created_date_spacy(doc.created_date, doc.ai_exception["why_archival"])
    assert result["score"] == 0
    assert result["reason"] == "Created date was not found in explanation."

    doc.ai_exception["why_archival"] += " The document was created on October 12 2024, which is recent history."
    result = deterministic_score.evaluate_created_date_spacy(doc.created_date, doc.ai_exception["why_archival"])
    assert result["score"] == 0
    assert result["reason"] == "Created date was not found in explanation."

    doc.ai_exception["why_archival"] += " The document was created on December 2024, which is recent history."
    result = deterministic_score.evaluate_created_date_spacy(doc.created_date, doc.ai_exception["why_archival"])
    assert result["score"] == 1
    assert result["reason"] == "Created date was found in explanation."
...
def test_updated_date_spacy():
    ...
    result = deterministic_score.evaluate_modified_date_spacy(doc.modification_date, doc.ai_exception["why_archival"])
    assert result["score"] == 1
    assert result["reason"] == "Modified date was not found in explanation or was within expected date range."

    doc.ai_exception["why_archival"] += " The document was created on October 12 2022, which is recent history."
    result = deterministic_score.evaluate_modified_date_spacy(doc.modification_date, doc.ai_exception["why_archival"])
    assert result["score"] == 0
    assert result["reason"] == "Modified date was found in explanation. It's a year beyond the created date or it mentions any date beyond the compliance deadline."
